...
export const HomeHeroSkeleton = ({ reflection }: MovieSkeleton) => {
  const cardCount = useBreakpoint()

  return (
    <>
      <div
        className={`relative grid grid-cols-1 md:grid-cols-3 place-items-center gap-6 min-h-[620px] pt-12 pb-64 px-5 sm:px-10 md:px-12 xl:px-24 2xl:px-48 animate-pulse ${
      ...
      {reflection && (
        <div className="h-0 px-5 sm:px-10 md:px-12 xl:px-24 2xl:px-48">
          <div className="card flex justify-center -translate-y-full gap-4 pt-8">
            {Array(cardCount)
              .fill({})
              .map((_, index) => (
                <div key={index}>
                  <div className="h-28  min-w-[156px] bg-gray-100 rounded"></div>
